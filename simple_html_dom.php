...
//        $pattern = "/([\w-:\*]*)(?:\#([\w-]+)|\.([\w-]+))?(?:\[@?(!?[\w-]+)(?:([!*^$]?=)[\"']?(.*?)[\"']?)?\])?([\/, ]+)/is";
	...
	*/
	...
	public function setAttribute($name, $value) : void {$this->__set($name, $value); }

	public function hasAttribute($name) {return $this->__isset($name); }

	public function removeAttribute($name) : void {$this->__set($name, null); }
...
class simple_html_dom
{
	...
	public function clear() : void
	{
		...
		if (isset($this->parent)) {$this->parent->clear(); $this->parent = null; }
		if (isset($this->root)) {$this->root->clear(); $this->root = null; }
		$this->doc = null;
		$this->noise = null;
	}
	...
	protected function prepare($str, $lowercase = true, $stripRN = true, $defaultBRText = DEFAULT_BR_TEXT, $defaultSpanText = DEFAULT_SPAN_TEXT) : void
	{
		$this->clear();
	...
	public function loadFile() : void {$args = func_get_args(); $this->load_file($args); }
}
